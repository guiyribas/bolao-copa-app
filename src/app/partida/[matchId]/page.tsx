...
import type { Match, PoolMatchBetsPayload, PoolMatchBetRow, Team } from '@/types';
...
function normalizePoolMatchBetsPayload(raw: unknown): PoolMatchBetsPayload | null {
  ...
  if (hasScores && homeTeam && awayTeam) {
    return (
      <div className="inline-flex flex-wrap items-center justify-center gap-1 sm:gap-1.5">
        ...
        <span className="tabular-nums font-medium text-neutral-900">{entry.homeScore}</span>
        <span className="text-neutral-400">×</span>
        <span className="tabular-nums font-medium text-neutral-900">{entry.awayScore}</span>
...
export default function PartidaPage() {
  ...
  return (
    <div className="max-w-3xl px-3 py-8">
      <PageBreadcrumb
        label={breadcrumbLabelForMatch(match)}
        className="mb-3"
      />
      ...
      {!poolData || poolData.pools.length === 0 ? (
        <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-6 text-sm text-neutral-700">
          <p className="mb-3">
            Você ainda não participa de nenhum bolão. Entre em um bolão para ver os palpites
            dos outros nesta partida, agrupados por bolão.
          </p>
          <Link
        ...
        <div className="flex flex-col gap-6">
          {poolData.pools.map((section) => (
            <section
              ...
              aria-labelledby={`pool-heading-${section.poolDocumentId}`}
            >
              <div className="border-b border-neutral-100 bg-neutral-50/80 px-3 py-2.5 flex flex-wrap items-baseline justify-between gap-2">
                <h2
                ...
                <Link
                  ...
                  className="text-xs font-medium text-emerald-800 underline decoration-emerald-200 underline-offset-2 hover:text-emerald-950"
                >
                  Ver ranking
                </Link>
              </div>
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-center tabular-nums text-neutral-600">
                          {entry.points != null ? (
                            <span
                              className={
                                entry.points === 0 ? 'text-red-600 font-medium' : 'text-green-700 font-medium'
                              }
                            >
