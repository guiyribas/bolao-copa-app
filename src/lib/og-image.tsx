...
function formatOgSubtitle(subtitle: string): { text: string; fontSize: number } {
  const text = subtitle.trim() || SITE_NAME;
  if (text.length > 48) {
    return { text: `${text.slice(0, 45)}…`, fontSize: 42 };
  }
  if (text.length > 32) {
...
export async function createSiteOgImage(subtitle: string) {
  ...
  const { text, fontSize } = formatOgSubtitle(subtitle);
